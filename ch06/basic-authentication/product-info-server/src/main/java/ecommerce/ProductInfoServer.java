...
public class ProductInfoServer {
  ...
  private void start() throws IOException {
    File certFile = Paths.get("ch06", "basic-authentication", "certs", "server.crt").toFile();
    File keyFile = Paths.get("ch06", "basic-authentication", "certs", "server.pem").toFile();
    File caFile = Paths.get("ch06", "basic-authentication", "certs", "ca.crt").toFile();
    ...
    server =
        NettyServerBuilder.forPort(PORT)
            .addService(
                ServerInterceptors.intercept(new ProductInfoImpl(), new BasicAuthInterceptor()))
            .sslContext(sslContext)
