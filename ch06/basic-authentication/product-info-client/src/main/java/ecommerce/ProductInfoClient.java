...
public class ProductInfoClient {
  ...
  public static void main(String[] args) throws SSLException {
    ...
    File certFile = Paths.get("ch06", "basic-authentication", "certs", "client.crt").toFile();
    File keyFile = Paths.get("ch06", "basic-authentication", "certs", "client.pem").toFile();

    SslContext sslContext =
        GrpcSslContexts.forClient().trustManager(caFile).keyManager(certFile, keyFile).build();
    ManagedChannel channel =
        NettyChannelBuilder.forAddress("localhost", 50001)
